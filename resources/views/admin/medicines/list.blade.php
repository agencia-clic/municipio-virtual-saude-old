...
<!-- actions - start -->
<div class="mt-3 mb-3">
    <span class="h5 text-800">Medicamentos</span>
            ...
            <div class="row flex-between-center">
                <div class="col-sm-auto mb-2 mb-sm-0">
                    <div class="btn-group btn-group-sm" role="group" aria-label="...">
                        <button class="btn btn-primary btn-sm" type="button" data-redirect="{{ route('medicines.form') }}"><span class="fas fa-plus"></span></button>
                        ...
                        <button class="btn btn-primary btn-sm collapsed" type="button" title="Limpar Filtros" data-redirect="{{ route('medicines') }}"><span class="fas fa-times"></span></button>
                    </div>
                </div>
...
<div class="card mb-3" id="customersTable" data-list=''>
    ...
    <div class="card-header">
        <div class="row flex-between-center">
            <div class="col-4 col-sm-auto d-flex align-items-center pe-0">
                <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">Medicamentos</h5>
            </div>
        </div>
    </div>
    ...
    <div class="accordion" id="accordionExample">
        <div class="accordion-item">
            <div class="accordion-collapse collapse {{ app('request')->input('medicines') ? "show" : ""}}" id="collapse2" aria-labelledby="heading2" data-bs-parent="#accordionExample">
                <div class="accordion-body">

                    <form id="formFiltro" action="{{ route('medicines') }}" method="get" enctype="multipart/form-data">
                        <div class="row">			
                            <div class="col-sm-6 col-md-4 col-lg-4 col-xl-4">
                                <div id="IdMedicines_campo" class="form-group">
                                    <label for="IdMedicines" id="label_IdMedicines">Código:</label>
                                    <input type="number" min="1" id="IdMedicines" name="IdMedicines" class="form-control form-control-sm" value="{{ app('request')->input('IdMedicines') }}" maxlength="11" autocomplete="off" />
                                </div>
                            </div>
                        ...
                        <input type="hidden" name="medicines" class="form-control form-control-sm" value="foo"/>
                ...
                @if(!empty($medicines->total()) AND ($medicines->total() > 0))
                    <thead class="bg-200 text-900">
                        <tr>
                            ...
                            <th class="sort pe-1 white-space-nowrap text-center" width="10%">Unidade</th>
                            <th class="sort pe-1 white-space-nowrap text-center" width="10%">Quantidade</th>
                    ...
                    <tbody class="list list-table" id="table-customers-body">
                        ...
                        @foreach($medicines as $val)
                            <tr class="btn-reveal-trigger" id="{{$val->IdMedicines}}-table">
                                ...
                                <td class="border email text-1000 py-2 text-center" width="10%">{{ $val->units }}</td>
                                <td class="border email text-1000 py-2 text-center" width="10%">{{ $val->amount }}</td>
                                <td class="border phone text-1000 white-space-nowrap py-2 text-end" width="8%">
                                    @if($val->status == "a")
                                    ...
                                    <div class="dropdown font-sans-serif position-static">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-end border py-0" aria-labelledby="customer-dropdown-2">
                                            <div class="bg-white py-2">
                                                <!-- edit -->
                                                <a class="dropdown-item fw-bold" href="{{route('medicines.form', ['IdMedicines' => base64_encode($val->IdMedicines)])}}"><span class="fas fa-edit me-1"></span><span> Editar</span></a>

                                                <!-- delete 
                                                <div class="dropdown-divider"></div>
                                                <a class="dropdown-item fw-bold" href="{ {route('medicines.form.delete', ['IdMedicines' => base64_encode($val->IdMedicines)])}}" data-id="{ { $val->IdMedicines }}" action="delete"><span class="fas fa-trash-alt me-1"></span><span> Deletar</span></a>-->
                                            </div>
                                        </div>
                                    </div>
                                </td>
