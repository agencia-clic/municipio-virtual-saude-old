...
<!-- actions - start -->
<div class="mt-3 mb-3">
    <span class="h5 text-800">Procedimentos</span>
    <span class="badge bg-primary">
        @if ($procedures->firstItem())
            <span class="font-medium">{{ $procedures->firstItem() }}</span>
            {!! __('até') !!}
            <span class="font-medium">{{ $procedures->lastItem() }}</span>
        @else
            {{ $procedures->count() }}
        @endif
        {!! __('de') !!}
        <span class="font-medium">{{ $procedures->total() }}</span>
        {!! __('registros') !!}
    </span>
</div>
            ...
            <div class="row flex-between-center">
                <div class="col-sm-auto mb-2 mb-sm-0">
                    <div class="btn-group btn-group-sm" role="group" aria-label="...">
                        <button class="btn btn-primary btn-sm" type="button" data-redirect="{{ route('procedures.form') }}"><span class="fas fa-plus"></span></button>
                        ...
                        <button class="btn btn-primary btn-sm collapsed" type="button" title="Limpar Filtros" data-redirect="{{ route('procedures') }}"><span class="fas fa-times"></span></button>
                    </div>
                </div>
...
<div class="card mb-3" id="customersTable" data-list=''>
    ...
    <div class="accordion" id="accordionExample">
        <div class="accordion-item">
            <div class="accordion-collapse collapse {{ app('request')->input('procedures') ? "show" : ""}}" id="collapse2" aria-labelledby="heading2" data-bs-parent="#accordionExample">
                <div class="accordion-body">

                    <form id="formFiltro" action="{{ route('procedures') }}" method="get" enctype="multipart/form-data">
                        <div class="row">			
                            <div class="col-sm-6 col-md-4 col-lg-4 col-xl-4">
                                <div id="IdProcedures_campo" class="form-group">
                                    <label for="IdProcedures" id="label_IdProcedures">Código:</label>
                                    <input type="number" min="1" id="IdProcedures" name="IdProcedures" class="form-control form-control-sm" value="{{ app('request')->input('IdProcedures') }}" maxlength="11" autocomplete="off" />
                                </div>
                            </div>
                        ...
                        <input type="hidden" name="procedures" class="form-control form-control-sm" value="foo"/>
    ...
    <div class="card-body p-0">
        <div class="table-responsive scrollbar">
            <table class="table table-sm table-striped fs--1 border mb-0 overflow-hidden">
                ...
                @if(!empty($procedures->total()) AND ($procedures->total() > 0))
                    ...
                    <tbody class="list list-table" id="table-customers-body">
                        ...
                        @foreach($procedures as $val)
                            <tr class="btn-reveal-trigger" id="{{$val->IdProcedures}}-table">
                                <td class="border email py-2 text-center" width="5%">{{ $val->code }}</td>
                                    ...
                                    <div class="dropdown font-sans-serif position-static">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-end border py-0" aria-labelledby="customer-dropdown-2">
                                            <div class="bg-white py-2">
                                                ...
                                                @can('roles', ['procedures.update'])
                                                <!-- edit -->
                                                <a class="dropdown-item fw-bold" href="{{route('procedures.form', ['IdProcedures' => base64_encode($val->IdProcedures)])}}"><span class="fas fa-edit me-1"></span><span> Editar</span></a>
                                                <div class="dropdown-divider"></div>
                                                @endcan

                                                @can('roles', ['procedures.delete'])
                                                <!-- delete -->
                                                <a class="dropdown-item fw-bold" href="{{route('procedures.form.delete', ['IdProcedures' => base64_encode($val->IdProcedures)])}}" data-id="{{ $val->IdProcedures }}" action="delete"><span class="fas fa-trash-alt me-1"></span><span> Deletar</span></a>
                                                @endcan
                                            </div>
                                        </div>
                                    </div>
    ...
    <!-- paginations -- start -->
    {{ $procedures->appends(app('request')->all())->links() }}
